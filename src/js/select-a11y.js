    ...
    const items = Array.prototype.map
      .call(
        ...
        function (option, index) {
          ...
          return `
        <li class="c-select-a11y__tag-item">
          <span>${text}</span>
          <button class="c-select-a11y__tag-item-supp" title="${this._options.text.deleteItem.replace(
            '{t}',
            text,
          )}" type="button" data-index="${index}">
            <span class="visually-hidden">${this._options.text.delete}</span>
            <span class="c-select-a11y__icon--delete" aria-hidden="true"></span>
          </button>
        </li>`;
        }.bind(this),
      )
